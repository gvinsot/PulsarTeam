...
export function agentSkillRoutes() {
  ...
  router.post('/', requireRole('admin', 'advanced'), asyncHandler(async (req, res) => {
  ...
  router.put('/:id', requireRole('admin', 'advanced'), asyncHandler(async (req, res) => {
  ...
  router.delete('/:id', requireRole('admin', 'advanced'), asyncHandler(async (req, res) => {
    const existing = await getAgentSkillById(req.params.id);
    if (!existing) return res.status(404).json({ error: 'Agent skill not found' });
