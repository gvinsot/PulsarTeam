...
export function boardRoutes(agentManager) {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const boardWorkflow =
        workflow && Array.isArray(workflow.columns) && workflow.columns.length > 0
      ...
      await logBoardAudit(board.id, 'create', req.user.userId, req.user.username, null, null, {
        name: boardName,
      });
  ...
  router.put(
    '/:id/plugins',
    validateBody(updatePluginsSchema),
    authorizeBoardAccess('edit'),
    asyncHandler(async (req, res) => {
  ...
  router.post(
    '/:id/plugins/assign',
    validateBody(pluginAssignSchema),
    authorizeBoardAccess('edit'),
    asyncHandler(async (req, res) => {
      const { board } = req.boardAccess;
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      await logBoardAudit(req.params.id, 'delete', req.user.userId, req.user.username, null, null, {
        name: board.name,
      });
