...
export function boardRoutes(agentManager) {
  ...
  router.get('/tasks/by-assignee/:agentId', asyncHandler(async (req, res) => {
  ...
  router.get('/:id', authorizeBoardAccess('read'), asyncHandler(async (req, res) => {
    const { board, permission, isOwner } = req.boardAccess;
    res.json({ ...board, _permission: permission, _isOwner: isOwner });
  }));

  // POST / — create a new board
  router.post('/', validateBody(createBoardSchema), asyncHandler(async (req, res) => {
  ...
  router.put('/:id', validateBody(updateBoardSchema), authorizeBoardAccess('edit'), asyncHandler(async (req, res) => {
  ...
  router.put('/:id/workflow', validateBody(updateWorkflowSchema), authorizeBoardAccess('edit'), asyncHandler(async (req, res) => {
  ...
  router.get('/:id/plugins', authorizeBoardAccess('read'), asyncHandler(async (req, res) => {
    const { board } = req.boardAccess;
    res.json({
  ...
  router.put('/:id/plugins', validateBody(updatePluginsSchema), authorizeBoardAccess('edit'), asyncHandler(async (req, res) => {
  ...
  router.post('/:id/plugins/assign', validateBody(pluginAssignSchema), authorizeBoardAccess('edit'), asyncHandler(async (req, res) => {
    const { board } = req.boardAccess;
  ...
  router.post('/:id/plugins/remove', validateBody(pluginAssignSchema), authorizeBoardAccess('edit'), asyncHandler(async (req, res) => {
    const { board } = req.boardAccess;
    const { pluginId } = req.body;
  ...
  router.put('/:id/mcp-auth', validateBody(mcpAuthSchema), authorizeBoardAccess('edit'), asyncHandler(async (req, res) => {
    const updated = await updateBoard(req.params.id, { mcp_auth: req.body });
    res.json(updated);
  }));

  // DELETE /:id — delete a board (owner or admin only)
  router.delete('/:id', authorizeBoardAccess('admin'), asyncHandler(async (req, res) => {
  ...
  router.get('/:id/shares', authorizeBoardAccess('admin'), asyncHandler(async (req, res) => {
    const shares = await getBoardShares(req.params.id);
    res.json(shares);
  }));

  // POST /:id/shares — share a board with a user (owner or admin)
  router.post('/:id/shares', validateBody(createShareSchema), authorizeBoardAccess('admin'), asyncHandler(async (req, res) => {
    const { board } = req.boardAccess;
  ...
  router.put('/:id/shares/:userId', validateBody(updateShareSchema), authorizeBoardAccess('admin'), asyncHandler(async (req, res) => {
  ...
  router.delete('/:id/shares/:userId', asyncHandler(async (req, res) => {
  ...
  router.get('/:id/audit', authorizeBoardAccess('admin'), asyncHandler(async (req, res) => {
