...
export function codeIndexRoutes(codeIndexService: CodeIndexService) {
  ...
  // Index an arbitrary server-side path. Admin only: the body is a free-form
  // absolute path on the API container's filesystem — unlike /index-project,
  // which is confined to REPOS_BASE_DIR — so any authenticated user of any
  // tenant could otherwise ingest whatever the container can read and then
  // search it back out through the /repos/:repoId/* readers.
  // Agents do NOT come through here: their index_folder tool goes to the
  // separate MCP handler mounted at /api/code-index/mcp (services/codeIndexMcp.ts).
  router.post(
    '/index-folder',
    requireRole('admin'),
  ...
  // Admin only: this WRITES caller-supplied content into a shared repo index
  // that every other tenant reads back through the search/outline routes.
  router.post(
    '/repos/:repoId/update-files',
    requireRole('admin'),
    asyncHandler(async (req, res) => {
      try {
        const params = repoParamsSchema.parse(req.params);
        ...
        if (!Array.isArray(files) || files.length === 0) {
          res.status(400).json({
            error: 'files array required (each entry: { path: string, content?: string })',
          });
          return;
        }
        if (files.length > 100) {
  ...
  // Destroy an indexed repo. Admin only: repo ids are enumerable through
  // GET /repos, so this was a one-request wipe of anyone else's index.
  router.delete(
    '/repos/:repoId',
    requireRole('admin'),
    asyncHandler(async (req, res) => {
