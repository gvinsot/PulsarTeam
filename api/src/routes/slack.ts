...
async function handleOAuthRedirect(req: express.Request, res: express.Response) {
  ...
  if (!config) {
    return sendOAuthResult(
      ...
      'slack-oauth-callback',
      false,
      'Slack not configured on server'
    );
  }
  ...
  try {
    ...
    console.log(
      `✅ [Slack] OAuth token stored for ${scopeType}:${scopeId} (team: ${data.team?.name || 'unknown'}) via redirect`
    );
    return sendOAuthResult(res, 'Slack', 'slack-oauth-callback', true, null, {
      teamName: data.team?.name,
    });
  } catch (err) {
...
const slackSpec: OAuthProviderSpec<{
  clientId: string;
  clientSecret: string;
  redirectUri: string;
}> = {
