...
const get = (path: string, opts: { auth?: boolean } = {}) =>
  apiFetch(`${API_BASE}${path}`, opts.auth === false ? {} : { headers: getHeaders() }).then(
    handleResponse
  );
...
const integration = (base: string) => ({
  status: (agentId?: string, boardId?: string) =>
    get(`/${base}/status${abQuery(agentId, boardId)}`),
  disconnect: (agentId?: string, boardId?: string) =>
    post(`/${base}/disconnect`, abBody(agentId, boardId)),
});
...
const oauthIntegration = (base: string) => ({
  ...
  authUrl: (agentId?: string, boardId?: string) =>
    get(`/${base}/auth-url${abQuery(agentId, boardId)}`),
});
...
const loginProvider = (base: string) => ({
  ...
  url: redirectUri =>
    get(`/auth/${base}/url?redirect_uri=${encodeURIComponent(redirectUri)}`, { auth: false }),
  callback: (code, redirectUri) =>
    post(`/auth/${base}/callback`, { code, redirect_uri: redirectUri }, { auth: false }),
});
...
export const api = {
  // Health
  getHealth: () => apiFetch(`${API_BASE}/health`).then(res => res.json()),
  ...
  completeTutorial: () => post('/auth/complete-tutorial'),
  ...
  getProjectStats: (days = 30) => get(`/tasks/project-stats?days=${days}`),

  createAgent: config => post('/agents', config, { long: true }),

  updateAgent: (id, updates) => put(`/agents/${id}`, updates),
  ...
  stopTask: taskId => post(`/tasks/${taskId}/stop`),

  chatAgent: (id, message) => post(`/agents/${id}/chat`, { message }, { long: true }),

  getHistory: id => get(`/agents/${id}/history`),

  reloadHistory: id => post(`/agents/${id}/history/reload`),

  clearHistory: id => del(`/agents/${id}/history`),

  getCodexAuthStatus: ownerId => get(`/codex-auth/${ownerId}/status`),

  uploadCodexAuth: (ownerId, authJson) =>
    post(`/codex-auth/${ownerId}`, { authJson }, { long: true }),

  deleteCodexAuth: ownerId => del(`/codex-auth/${ownerId}`),

  reloadContext: id => post(`/agents/${id}/reload-context`),

  restartRuntime: id => post(`/agents/${id}/restart`, undefined, { long: true }),

  truncateHistory: (id, afterIndex) => del(`/agents/${id}/history/after/${afterIndex}`),

  handoff: (fromId, targetAgentId, context) =>
    post(`/agents/${fromId}/handoff`, { targetAgentId, context }, { long: true }),

  broadcast: message => post('/agents/broadcast/all', { message }, { long: true }),
  ...
  setTaskStatus: (agentId, taskId, status) => patchAgentTask(agentId, taskId, { status }),

  updateTask: (agentId, taskId, fields) => patchAgentTask(agentId, taskId, fields),
  ...
  // RAG
  addRagDoc: (agentId, name, content) => post(`/agents/${agentId}/rag`, { name, content }),

  addRagUrl: (agentId, name, url) => post(`/agents/${agentId}/rag/url`, { name, url }),

  refreshRagDoc: (agentId, docId) => post(`/agents/${agentId}/rag/${docId}/refresh`),

  deleteRagDoc: (agentId, docId) => del(`/agents/${agentId}/rag/${docId}`),

  // Plugins (unified skills + MCP)
  getPlugins: () => get('/plugins'),

  createPlugin: config => post('/plugins', config),

  updatePlugin: (id, updates) => put(`/plugins/${id}`, updates),

  deletePlugin: id => del(`/plugins/${id}`),

  // Agent plugin assignment
  assignPlugin: (agentId, pluginId) => post(`/agents/${agentId}/plugins`, { pluginId }),

  removePlugin: (agentId, pluginId) => del(`/agents/${agentId}/plugins/${pluginId}`),

  // MCP Servers
  getMcpServers: () => get('/mcp-servers'),

  connectMcpServer: id => post(`/mcp-servers/${id}/connect`),

  testMcpServer: (id, apiKey) => post(`/mcp-servers/${id}/test`, apiKey ? { apiKey } : undefined),
  ...
  // Realtime (Voice)
  getRealtimeToken: agentId => post('/realtime/token', { agentId }),

  // External voice (STT + LLM + TTS pipeline) — returns WSS URLs for the browser
  getExternalVoiceConfig: agentId => get(`/external-voice/config/${encodeURIComponent(agentId)}`),
  ...
  testExternalVoiceService: (service: 'stt' | 'tts', url?: string, apiKey?: string) =>
    post(`/external-voice/test/${service}`, {
      ...(url ? { url } : {}),
  ...
  resetInstructionsByRole: role => post(`/agents/reset-instructions/${encodeURIComponent(role)}`),
  ...
  attachBoardToProject: (projectId, boardId) => post(`/projects/${projectId}/boards/${boardId}`),

  detachBoardFromProject: (projectId, boardId) => del(`/projects/${projectId}/boards/${boardId}`),

  // Storages accessible via the board's OneDrive plugin OAuth token (picker source)
  getBoardAvailableStorages: boardId => get(`/projects/boards/${boardId}/available-storages`),

  // Available repos from configured git connections (for the picker)
  getAvailableRepos: () => get('/projects/available-repos'),

  // Available repos via the board's GitHub plugin OAuth token (for BoardReposPanel)
  getBoardAvailableRepos: boardId => get(`/projects/boards/${boardId}/available-repos`),

  // Code Index — auto-index project by name
  indexProject: projectName => post('/code-index/index-project', { projectName }, { long: true }),
  ...
  analyzeCodeGraph: (
    ...
    repo,
    boardId,
    { direction = 'ui-to-service', ref = 'main', refresh = false } = {}
  ) =>
    post(
  ...
  // API Key (MCP)
  getApiKeyInfo: () => get('/settings/api-key'),

  generateApiKey: () => post('/settings/api-key'),

  revokeApiKey: () => del('/settings/api-key'),

  // General settings
  getSettings: () => get('/settings/general'),

  updateSettings: patch => put('/settings/general', patch),

  // Reminder configuration
  getReminderConfig: () => get('/settings/general/reminders'),

  updateReminderConfig: patch => put('/settings/general/reminders', patch),

  // Boards (per-user multi-board)
  getBoards: () => get('/boards'),

  getAllBoardsAdmin: () => get('/boards/all'),

  createBoard: (name, workflow, filters) => post('/boards', { name, workflow, filters }),

  updateBoard: (id, updates) => put(`/boards/${id}`, updates),

  updateBoardWorkflow: (id, workflow) => put(`/boards/${id}/workflow`, workflow),

  deleteBoard: id => del(`/boards/${id}`),

  // Board plugins
  getBoardPlugins: boardId => get(`/boards/${boardId}/plugins`),

  assignBoardPlugin: (boardId, pluginId) => post(`/boards/${boardId}/plugins/assign`, { pluginId }),

  removeBoardPlugin: (boardId, pluginId) => post(`/boards/${boardId}/plugins/remove`, { pluginId }),

  // Board sharing
  getBoardShares: boardId => get(`/boards/${boardId}/shares`),
  ...
  removeBoardShare: (boardId, userId) => del(`/boards/${boardId}/shares/${userId}`),

  getBoardUsers: () => get('/boards/users'),

  // Project task stats
  getProjectTaskStats: project => get(`/agents/tasks/stats?project=${encodeURIComponent(project)}`),
  ...
  getGlobalAgentTime: (days = 30) => get(`/agents/tasks/stats/agent-time?days=${days}`),
  ...
  updateUser: (id, updates) => put(`/users/${id}`, updates),
  ...
  createLlmConfig: data => post('/llm-configs', data),

  updateLlmConfig: (id, data) => put(`/llm-configs/${id}`, data),

  deleteLlmConfig: id => del(`/llm-configs/${id}`),
...
// Budget
export const fetchBudgetSummary = (days = 1) => get(`/budget/summary?days=${days}`);

export const fetchBudgetByAgent = (days = 30) => get(`/budget/by-agent?days=${days}`);

export const fetchBudgetTimeline = (days = 7, groupBy = 'day') =>
  get(`/budget/timeline?days=${days}&groupBy=${groupBy}`);

export const fetchBudgetDaily = (days = 30) => get(`/budget/daily?days=${days}`);

export const fetchBudgetConfig = () => get('/budget/config');

export const updateBudgetConfig = config => put('/budget/config', config);

export const fetchBudgetAlerts = () => get('/budget/alerts');

export default api;

/* ── Task CRUD (board-level, uses /api/tasks/:id) ─────────────────────────── */
export const updateTask = (taskId, fields) => put(`/tasks/${taskId}`, fields);

export const deleteTask = taskId => del(`/tasks/${taskId}`);

export const clearTaskStopped = taskId => patch(`/tasks/${taskId}/clear-stopped`);

/* ── Reorder tasks within a column ──────────────────────────────────── */
export const reorderTasks = orderedIds => put('/tasks/reorder', { orderedIds });

/* ── Soft-delete management ──────────────────────────────────────────────── */
export const getDeletedTasks = () => get('/tasks/deleted');

export const restoreTask = taskId => post(`/tasks/${taskId}/restore`);

export const hardDeleteTask = taskId => del(`/tasks/${taskId}/permanent`);

/* ── Commit diff ──────────────────────────────────────────────────────────── */
export const getCommitDiff = (taskId, hash) => get(`/tasks/${taskId}/commits/${hash}/diff`);
