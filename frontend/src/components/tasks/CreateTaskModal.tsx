...
export default function CreateTaskModal({
  ...
  const initialStatus =
    defaultStatus && CREATE_STATUSES.some(s => s.value === defaultStatus)
      ? defaultStatus
          ...
          <div>
            ...
            {repoLoadError ? (
              ...
              <select
                value={repoFullName}
                onChange={e => {
                  userTouchedRepo.current = true;
                  const v = e.target.value;
                  setRepoFullName(v);
                  setSecondaryRepos(prev => prev.filter(s => s !== v));
                }}
          ...
          {repoFullName && availableRepos.length > 0 && (
            <div>
              ...
              <select
                ...
                onChange={e => {
                  const v = e.target.value;
                  if (v && !secondaryRepos.includes(v)) setSecondaryRepos([...secondaryRepos, v]);
                }}
                className="w-full px-3 py-2 bg-dark-800 border border-dark-700 rounded-lg text-sm text-dark-200 focus:outline-none focus:border-indigo-500 transition-colors"
              >
              ...
              {secondaryRepos.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {secondaryRepos.map(fn => (
                    <span
                      key={fn}
                      className="inline-flex items-center gap-1 px-2 py-1 text-[11px] rounded-md bg-emerald-500/10 text-emerald-300 border border-emerald-500/30"
                    >
                      {fn}
                      <button
                        type="button"
                        onClick={() => setSecondaryRepos(secondaryRepos.filter(s => s !== fn))}
          ...
          <div>
            ...
            {storageLoading ? (
              ...
              <select
                value={storagePath}
                onChange={e => {
                  userTouchedStorage.current = true;
              ...
              >
                ...
                {availableStorages.map(s => (
                  <option key={`${s.provider}:${s.path}`} value={s.path}>
                    [{s.provider}] {s.displayName || s.path}
                  </option>
                ))}
              </select>
            ) : (
