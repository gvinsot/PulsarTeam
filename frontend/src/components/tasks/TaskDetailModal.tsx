          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative" ref={refineRef}>
                      <button
                        ...
                        ${refining ? 'text-amber-300' : 'text-dark-500 hover:text-amber-400'}`}
                      >
                        {refining ? (
                          <>
                            <svg
                      ...
                      {refineOpen && !refining && (
                        <div
                          className="absolute right-0 top-6 z-50 bg-dark-800 border border-dark-600
                        ...
                        >
                          ...
                          {(agents || []).filter(a => a.enabled !== false && a.status === 'idle')
                            .length === 0 && (
                          ...
                          {(agents || [])
                            .filter(a => a.enabled !== false && a.status === 'idle')
                            .map(a => (
                              <button
                                key={a.id}
                                onClick={async () => {
                    ...
                    components={{
                      ...
                      code: ({ children }: { children?: ReactNode }) =>
                        !String(children).includes('\n') ? (
                          <code className="bg-dark-700 px-1.5 py-0.5 rounded text-purple-300 text-xs">
                            {children}
                          </code>
                        ) : (
                      ...
                      table: ({ children }) => (
                        <table className="border-collapse border border-dark-600 my-2 w-full text-xs">
                          {children}
                        </table>
                      ),
                      th: ({ children }) => (
                        <th className="border border-dark-600 px-2 py-1 bg-dark-700 text-left">
                          {children}
                        </th>
                      ),
                      td: ({ children }) => (
                      ...
                      h1: ({ children }) => (
                        <h1 className="text-lg font-bold text-dark-100 mt-3 mb-1">{children}</h1>
                      ),
                      h2: ({ children }) => (
                        <h2 className="text-base font-bold text-dark-100 mt-3 mb-1">{children}</h2>
                      ),
                      h3: ({ children }) => (
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex items-center justify-between py-2 border-b border-dark-800">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <span className="text-[10px] text-dark-500">
                    {task.isManual ? 'Not processed by agents' : 'Processed by agents'}
                  </span>
                  <button
                    onClick={async () => {
              ...
              <EditableSelectRow
                ...
                pencilReplacement={
                  repoLoadError ? (
                    <span className="text-[10px] text-amber-400 italic" title={repoLoadError}>
                      github plugin off
                    </span>
                  ) : null
                }
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(() => {
                      ...
                      const addable = availableRepos.filter(
                        r => r.fullName !== task.repoFullName && !currentNames.includes(r.fullName)
                      );
                      return (
                        <div className="flex flex-wrap items-center gap-1.5">
                          {current.map(r => (
                            <span
                              key={r.fullName}
                              className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/20"
                            >
                              {r.fullName}
                              <button
                                type="button"
                                title="Remove"
                                onClick={() =>
                                  saveSecondaryRepos(current.filter(x => x.fullName !== r.fullName))
                                }
                              ...
                              onChange={e => {
                                ...
                                if (v)
                                  saveSecondaryRepos([
                                    ...
                                    {
                                      provider:
                                        availableRepos.find(r => r.fullName === v)?.provider ||
                                        'github',
                                      fullName: v,
                                    },
                                  ]);
              ...
              <EditableSelectRow
                ...
                options={availableStorages.map(s => ({
                  key: `${s.provider}:${s.path}`,
                  value: s.path,
                  label: `[${s.provider}] ${s.displayName || s.path}`,
                }))}
                onSave={saveStorage}
                disableWhenEmpty
                ...
                pencilReplacement={
                  !storageLoading && (storageLoadError || availableStorages.length === 0) ? (
                ...
                view={
                  task.taskType && TASK_TYPE_MAP[task.taskType] ? (
                    (() => {
                      ...
                      return (
                        <span
                          className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium ring-1 ${tt.cls}`}
                        >
                  ...
                  {editingBoard ? (
                    <div className="flex items-center gap-1.5">
                      <select
                        ...
                        onChange={e => {
                          ...
                          if (targetId === (task.boardId || activeBoardId)) {
                            setEditingBoard(false);
                            return;
                          }
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        title={
                          task.actionRunning
                            ? 'Stop the agent first to move the task'
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-dark-400 text-xs mt-0.5">
                      The task will be moved and placed in the first column
                    </p>
                  </div>
                </div>
