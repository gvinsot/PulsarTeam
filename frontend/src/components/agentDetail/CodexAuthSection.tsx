...
export default function CodexAuthSection({
  ...
  const handleFile = async (file: File) => {
    ...
    if (file.size > 256 * 1024) {
      setUploadError('File too large (>256 KB)');
      return;
    }
    setUploading(true);
    try {
  ...
  const handleDelete = async () => {
    if (!ownerId) return;
    if (!confirm('Forget the stored Codex auth.json? Agents will fall back to OPENAI_API_KEY.'))
      return;
    try {
      await api.deleteCodexAuth(ownerId);
      await refresh();
    } catch (err) {
      console.error(err);
    }
  ...
  const planLabel = status?.authenticated
    ? status.plan === 'chatgpt-oauth'
      ? 'ChatGPT plan (OAuth)'
      : status.plan === 'api-key'
        ? 'OPENAI_API_KEY (uploaded)'
        : 'auth.json present'
    : 'No auth uploaded';
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[11px] text-dark-500 mt-0.5 truncate">
                ...
                {status?.updatedAt
                  ? ` · refreshed ${new Date(status.updatedAt).toLocaleString()}`
                  : ''}
              </div>
            </div>
          </div>
      ...
      {showModal && (
        ...
        >
          <div
            ...
            className="bg-dark-900 border border-dark-700 rounded-xl max-w-lg w-full p-5"
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <KeyRound className="w-4 h-4 text-amber-400" />
                <h3 className="text-base font-semibold text-dark-100">
                  Login with your ChatGPT plan
                </h3>
              </div>
              <button
                onClick={() => setShowModal(false)}
            ...
            <div className="text-sm text-dark-300 space-y-3">
              <p>
                The ChatGPT OAuth flow is not exposed to third-party browsers, so the login step
                happens once on your own machine. The runner only needs the resulting
                <code className="mx-1 px-1.5 py-0.5 bg-dark-800 rounded text-xs text-amber-300">
                  auth.json
                </code>
                file.
              </p>
              <ol className="list-decimal list-inside text-xs text-dark-300 space-y-1.5 bg-dark-800/50 rounded-lg p-3 border border-dark-700/50">
                <li>
                  ...
                  <code className="text-amber-300">npm i -g @openai/codex</code>
                </li>
                <li>
                  Run <code className="text-amber-300">codex login</code> — a browser opens, sign in
                  with your ChatGPT account.
                </li>
                <li>
                  Locate the generated file at{' '}
                  <code className="text-amber-300">~/.codex/auth.json</code>.
                </li>
              ...
              {status?.authenticated && !uploading && !uploadError && (
                <div className="flex items-center gap-2 text-xs text-emerald-300">
                  <Check className="w-3.5 h-3.5" /> A token is already stored — uploading replaces
                  it.
                </div>
              )}
            </div>
