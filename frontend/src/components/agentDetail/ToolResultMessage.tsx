...
function RichToolOutput({
  ...
  if (isGitOutput(tool, args, output)) {
    ...
    return (
      <div
        className={`mt-1 ml-3 rounded text-[11px] overflow-x-auto max-h-80 overflow-y-auto border ${
        ...
        <div className="p-2 font-mono leading-relaxed">
          {lines.map((line, i) => {
            ...
            return (
              <div
                key={i}
                className={`whitespace-pre-wrap break-all px-1 ${cls === 'commit' && i > 0 ? 'mt-3 pt-2 border-t border-dark-700/50' : ''} ${gitLineStyles[cls]}`}
              >
                {line || '\u00A0'}
              </div>
            );
  ...
  return (
    <pre
      className={`mt-1 ml-3 p-2 rounded text-[11px] overflow-x-auto max-h-80 overflow-y-auto whitespace-pre-wrap break-all ${
        success
...
function ToolResultItem({ result }: { result: MessageToolResult }) {
  ...
  return (
    <div className="text-xs">
      ...
      >
        <span
          className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${result.success ? 'bg-emerald-500' : 'bg-red-500'}`}
        />
        <code className={`font-mono ${result.success ? 'text-dark-300' : 'text-red-300'}`}>
          @{result.tool}({argSummary})
        </code>
        {hasContent &&
          (showOutput ? (
