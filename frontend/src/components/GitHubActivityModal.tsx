...
export default function GitHubActivityModal({
  ...
  const [tab, setTab] = useState<'activity' | 'explorer' | 'callgraph'>('activity');

  // Build a sha → tags lookup so each commit can render its tags inline
  const tagsBySha = useMemo(() => {
    const map = new Map<string, GitHubActivityTag[]>();
  ...
  // GitHubActivityCommit.date is optional (the producer drops the key when
  // neither the author nor the committer date exists), hence the optional param.
...
function RepoExplorer({ owner, repo, boardId }: { owner: string; repo: string; boardId: string }) {
  ...
  const [fileContent, setFileContent] = useState<RepoExplorerFileState>(null);
  ...
  // `fileContent` holds either the API payload or the locally-synthesised
  // { error } object from the catch above (RepoExplorerFileState). Split the
  // union once here: the 200 body never carries an `error` key, and the error
  // object never carries any of the payload keys, so each branch below reads
  // exactly the same values it read before.
  const fileError = fileContent && 'error' in fileContent ? fileContent.error : null;
  const filePayload = fileContent && !('error' in fileContent) ? fileContent : null;
...
function FileTreeView({ nodes, expandedDirs, toggleDir, openFile, depth }: FileTreeViewProps) {
  return (
    <div>
      {nodes.map(node => (
        <FileTreeNode
...
function FileTreeNode({
  ...
  openFile,
  depth,
}: Omit<FileTreeViewProps, 'nodes'> & { node: RepoFileTreeNode }) {
...
function buildNestedTree(flatTree: GitHubTreeEntry[]): RepoFileTreeNode[] {
  const root: RepoFileTreeNode[] = [];
  const dirMap = new Map<string, RepoFileTreeNode>();
  ...
  for (const item of sorted) {
    ...
    if (parts.length === 1) {
      ...
      // dirMap only ever holds 'tree' nodes, which are built with `children: []`,
      // so a resolved parent always has a children array. Reading it up front
      // keeps the existing "no parent → push to root" fallback as the single
      // fallback rather than asserting the array exists.
      const parentChildren = dirMap.get(parentPath)?.children;
      if (parentChildren) {
  ...
  const sortChildren = (nodes: RepoFileTreeNode[]) => {
    nodes.sort((a, b) => {
      if (a.type === 'tree' && b.type !== 'tree') return -1;
      if (a.type !== 'tree' && b.type === 'tree') return 1;
...
// `ext` is undefined when the name has no '.' segment — String.split().pop()
// can also come back undefined, and both cases fall through to the default.
function getFileIconColor(ext: string | undefined, isDir: boolean) {
  ...
  return (ext && colors[ext]) || 'text-dark-300';
}
