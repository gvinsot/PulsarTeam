...
export default function SettingsTab({ active, showToast }) {
  ...
  const handleTestVoiceService = useCallback(
    async (service: 'stt' | 'tts') => {
      ...
      try {
        ...
        if (result?.ok) {
          showToast?.(
            `${service.toUpperCase()} connected${result.latencyMs != null ? ` (${result.latencyMs} ms)` : ''}`,
            'success'
          );
        } else {
          showToast?.(
            `${service.toUpperCase()} test failed: ${result?.error || 'unknown error'}`,
            'error'
          );
        }
      } catch (err: any) {
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-dark-500 mt-1">
                Min wait after a reminder before next
              </p>
            </div>
          </div>
        ) : (
      ...
      <div className="p-5 bg-dark-800 rounded-xl border border-dark-700 space-y-3">
        <div>
          ...
          <p className="text-xs text-dark-400 mt-1">
            Optional LLM used to simplify the repository call-graph analysis (UI ↔ services). When
            unset, the graph is built deterministically from source-file parsing only.
          </p>
        </div>
        <select
      ...
      <div className="p-5 bg-dark-800 rounded-xl border border-dark-700 space-y-3">
        <div>
          ...
          <p className="text-xs text-dark-400 mt-1">
            ...
            <span className="text-emerald-300">🤖 Automatic</span>. It reads the task and the roles
            available on the board, then picks the best-fit role to run the action. When unset, such
            actions fail with a hint (unless the board has a single role, which is used directly).
          </p>
        </div>
        <select
      ...
      <div className="p-5 bg-dark-800 rounded-xl border border-dark-700 space-y-3">
        <div>
          ...
          <p className="text-xs text-dark-400 mt-1">
            The Claude CLI is driven through a PTY (no <code>-p</code>) to keep subscription
            pricing. When the TUI shows a Y/N or list prompt we don't have a hardcoded answer for,
            this LLM is consulted to choose the safest option. Leave unset to fall back to safe
      ...
      <div className="p-5 bg-dark-800 rounded-xl border border-dark-700 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-dark-400 mt-1">
              The browser streams microphone audio (PCM16 mono @ 16 kHz) to this WebSocket and
              receives transcripts back. Used both in regular agent chat (mic input) and by
              external-voice agents. Compatible with{' '}
              <a
                className="underline"
                href="https://speech-ui.methodinfo.fr/"
                target="_blank"
                rel="noreferrer"
              >
                HighSpeedToText
      ...
      <div className="p-5 bg-dark-800 rounded-xl border border-dark-700 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-dark-400 mt-1">
              The browser sends text to this WebSocket and streams back PCM16 mono @ 22 050 Hz. Used
              both for spoken-reply in agent chat (when enabled per-agent) and by external-voice
              agents. Can use a different provider from STT.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="md:col-span-2">
            ...
            <p className="text-[10px] text-dark-500 mt-1">
              ...
              Per-agent voice IDs can override this default.
            </p>
          </div>
        </div>
        {voiceTest.tts && (
      ...
      <div className="p-5 bg-dark-800 rounded-xl border border-dark-700 space-y-4">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    showToast?.(
                      `Reset ${res.resetCount} agent(s) with role "${resetRole}" to default instructions`,
                      'success'
                    );
                    setResetConfirm(false);
                  } catch (err) {
        ...
        {resetConfirm && resetRole && (
          <div className="text-xs px-3 py-2 rounded-lg bg-red-900/20 text-red-400 border border-red-800/30">
            This will overwrite the instructions of <strong>all agents</strong> with role "
            {resetRole}" — this cannot be undone.
          </div>
        )}
      </div>
