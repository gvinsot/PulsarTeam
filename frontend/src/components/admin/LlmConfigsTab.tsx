    ...
    <>
          ...
          {llmConfigs.map(config => (
            ...
            >
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  {config.managesContext && (
                    <span className="ml-1 px-1.5 py-0.5 text-[10px] rounded bg-teal-500/10 text-teal-400 border border-teal-500/20">
                      Managed Context
                    </span>
                  )}
                  {config.endpoint && (
                  ...
                  {config.contextSize && (
                    <span className="text-xs text-dark-500">
                      {(config.contextSize / 1000).toFixed(0)}k ctx
                    </span>
                  )}
                  {config.maxOutputTokens && (
                    <span className="text-xs text-dark-500">
                      {(config.maxOutputTokens / 1000).toFixed(0)}k out
                    </span>
                  )}
                  {config.temperature != null && (
                    <span className="text-xs text-dark-500">temp {config.temperature}</span>
                  )}
                  {config.costPerInputToken != null && (
                    <span className="text-xs text-dark-500">
                      ${config.costPerInputToken}/{config.costPerOutputToken} per 1M
                    </span>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <button
                  onClick={() => handleDeleteLlmConfig(config)}
      ...
      <div className="p-4 bg-dark-800/50 rounded-xl border border-dark-700">
        <p className="text-xs text-dark-400">
          LLM configurations are shared across the team. When you assign an LLM to an agent, it uses
          the provider, model, and API key from this configuration. Agents can still override the
          API key in their own settings.
        </p>
      </div>
    </>
  );
